...
export class PortlandMaps {
  ...
  suggestAddress(query, options, success, failure) {
    ...
    return axios.post(SUGGEST_URL, params).then((value) => {
      if(value.data && value.data.status === 'success' && value.data.candidates) {
        if(value.data.candidates[0]) {
          ...
          failure(`Portlandmaps.com could not suggest a location for "${query}"`);
        }
      }
